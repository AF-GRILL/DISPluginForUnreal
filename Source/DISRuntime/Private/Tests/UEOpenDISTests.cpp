			...
			for (int Index = 0; Index < NumTestCases; Index++)
			{
				...
				It(FString::Printf(
					   ...
					   TestCase.EcefY, TestCase.EcefZ).Replace(TEXT("."), TEXT(",")), [this, TestCase, ECEF, &latLonHeight]()
				   {
						...
						TestEqual(FString::Printf(TEXT("Calculated Latitude %7.4f° = Desired Latitude %7.4f°"), latLonHeight.X, TestCase.LatitudeDegrees), latLonHeight.X, TestCase.LatitudeDegrees, 1.05);
						TestEqual(FString::Printf(TEXT("Calculated Longitude %7.4f° = Desired Longitude %7.4f°"), latLonHeight.Y, TestCase.LongitudeDegrees), latLonHeight.Y, TestCase.LongitudeDegrees, 1.05);
						TestEqual(FString::Printf(TEXT("Calculated Height %7.4fm = Desired Height %7.4fm"), latLonHeight.Z, TestCase.HeightMeters), latLonHeight.Z, TestCase.HeightMeters, 23);
				   });
			}
		});
	...
	Describe("CalculateEcefXYZ", [this]()
		{
			for (int Index = 0; Index < NumTestCases; Index++)
			{
				...
				FVector latLonHeight;
				latLonHeight.X = TestCase.LatitudeDegrees;
				latLonHeight.Y = TestCase.LongitudeDegrees;
				latLonHeight.Z = TestCase.HeightMeters;
